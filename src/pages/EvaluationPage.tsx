...
export default function EvaluationPage({ projectId, onNext }: EvaluationPageProps) {
  ...
  const metricCards = evalData ? [
    { label: 'Accuracy', value: fmt(evalData.accuracy) },
    { label: 'Precision', value: fmt(evalData.precision) },
    { label: 'Recall', value: fmt(evalData.recall) },
    { label: 'F1-Score', value: fmt(evalData.f1_score) },
    { label: 'Best Model', value: evalData.best_model ?? '—' },
  ] : []
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#1e2a3a] bg-[#0d1117]">
                    {['Model', 'Task', 'Accuracy', 'Precision', 'Recall', 'F1'].map(h => (
