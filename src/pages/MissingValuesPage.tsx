...
type NumericMethod = 'mean' | 'median' | 'mode' | 'drop'
type CatMethod = 'mode' | 'drop'

const NUMERIC_OPTIONS: { label: string; value: NumericMethod }[] = [
...
const CAT_OPTIONS: { label: string; value: CatMethod }[] = [
  { label: 'Mode (most frequent)', value: 'mode' },
  { label: 'Drop Row', value: 'drop' },
]

export default function MissingValuesPage({ projectId, onNext }: MissingValuesPageProps) {
  const [numMethod, setNumMethod] = useState<NumericMethod>('mean')
  const [catMethod, setCatMethod] = useState<CatMethod>('mode')
  ...
  const missingCols = useMemo(() => {
    ...
    return info.columns
      ...
      .map(col => ({
        ...
        appliedMethod: categoricalSet.has(col) ? catMethod : numMethod,
      }))
  }, [info, categoricalSet, numMethod, catMethod])

  const applyMutation = useMutation({
    mutationFn: () =>
      preprocessingApi.missingValues(projectId, {
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '64px' }}>
      <div className="p-6">
        <div className="mb-6">
          <p className="text-sm text-[#64748b]">Configure imputation strategies for columns with missing data.</p>
        </div>
        ...
        {/* Global Strategy Selectors */}
        <div className="grid grid-cols-2 gap-4 mb-6">
          {/* Numeric method */}
          <div className="bg-[#111827] border border-[#1e2a3a] rounded-lg p-4">
            <p className="text-[10px] font-semibold text-[#64748b] uppercase tracking-widest mb-3">Numeric Columns Strategy</p>
            <div className="relative">
              <button
                onClick={() => { setNumOpen(v => !v); setCatOpen(false) }}
              ...
              {numOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-[#1c2333] border border-[#2d3748] rounded shadow-xl z-20">
                  {NUMERIC_OPTIONS.map(o => (
                    <button key={o.value} onClick={() => { setNumMethod(o.value); setNumOpen(false) }}
          ...
          {/* Categorical method */}
          <div className="bg-[#111827] border border-[#1e2a3a] rounded-lg p-4">
            <p className="text-[10px] font-semibold text-[#64748b] uppercase tracking-widest mb-3">Categorical Columns Strategy</p>
            <div className="relative">
              <button
                onClick={() => { setCatOpen(v => !v); setNumOpen(false) }}
                className="w-full flex items-center justify-between px-3 py-2.5 bg-[#1c2333] border border-[#2d3748] rounded text-sm text-[#e2e8f0] hover:border-[#374151]"
              >
              ...
              {catOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-[#1c2333] border border-[#2d3748] rounded shadow-xl z-20">
                  {CAT_OPTIONS.map(o => (
                    <button key={o.value} onClick={() => { setCatMethod(o.value); setCatOpen(false) }}
            ...
            <table className="w-full min-w-[560px] text-sm">
              <thead>
                <tr className="border-b border-[#1e2a3a] bg-[#0d1117]">
                  {['Column Name', 'Type', 'Missing Count', 'Percentage', 'Strategy'].map(h => (
              ...
              <tbody>
                {isLoading && (
                  <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-[#64748b]">Loading columns…</td></tr>
                )}
                {!isLoading && missingCols.length === 0 && (
                  <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-[#22c55e]">No missing values found — dataset is complete!</td></tr>
                )}
                {missingCols.map(col => (
                  <tr key={col.name} className="border-b border-[#1e2a3a] hover:bg-[#0d1117]">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-20 bg-white/[0.08] rounded-full h-[4px]">
                    ...
                    <td className="px-5 py-3">
                      <span className="px-2 py-1 bg-[#1c2333] border border-[#2d3748] rounded text-xs text-[#e2e8f0]">
                        {col.appliedMethod}
                      </span>
                    </td>
                  </tr>
