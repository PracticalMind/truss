...
type Operation = 'arithmetic' | 'transform' | 'binning'
...
export default function FeatureEngineeringPanel({ projectId, onApplied }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`Column "${newCol}" created`)
      setNewCol('')
      onApplied()
    },
  ...
  const canApply = newCol.trim().length > 0 && (
    operation === 'arithmetic' ? (!!colA && !!colB) :
    operation === 'transform'  ? !!singleCol :
    !!singleCol
  )
